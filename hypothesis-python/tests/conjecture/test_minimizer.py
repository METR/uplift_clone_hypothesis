...
def test_shrink_strings(initial, predicate, intervals, expected):
    assert String.shrink(initial, predicate, intervals=intervals, min_size=len(expected)) == tuple(expected)
...
def test_collection_left_is_better():
    shrinker = Collection([1, 2, 3], lambda v: True, ElementShrinker=Integer, min_size=3)
