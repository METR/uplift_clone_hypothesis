...
TYPES = sorted(
    (x for x in _global_type_lookup if x.__module__ != "typing" and x.__name__ != "ByteString"),
    key=str,
)
...
@given(
    excluded_types=st.lists(st.sampled_from(TYPES), min_size=1, max_size=3, unique=True).map(tuple),
    data=st.data(),
)
